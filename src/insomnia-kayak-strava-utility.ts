...
export const authenticate = async (path:string, 
                                    REACT_APP_CLIENT_ID:string, 
                                    REACT_APP_CLIENT_SECRET:string)  =>  {
    console.log("Let's see what we can do");
    try {
      // If not redirected to Strava, return to home
      if (_.isEmpty(path)) {
        console.log(`empty location ${path} `)
        return "/";
      }
      console.log(`Moving on with empty ${path}.`);
      // Save the Auth Token to the Store (it's located under 'search' for some reason)
      const stravaAuthToken = cleanUpAuthToken(location.search);

      // Post Request to Strava (with AuthToken) which returns Refresh Token and and Access Token
      const tokens = await testAuthGetter(stravaAuthToken, REACT_APP_CLIENT_ID, REACT_APP_CLIENT_SECRET);
      
      //props.setUser(tokens);
      const accessToken = tokens.access_token;
      console.log(`accessTokens is ${JSON.stringify(accessToken)}`)
     ...
     console.log("Nothing else happened");
    return "/";
};
...
const testAuthGetter = async (authTok, REACT_APP_CLIENT_ID, REACT_APP_CLIENT_SECRET): Promise<Token> => {
  try {
